...
Router.prototype.go = function (routeNameOrPath, params, options) {
  ...
  if (isPath.test(routeNameOrPath)) {
    ...
    var urlParts = Meteor.absoluteUrl().split('/');
    var isRootPath = routeNameOrPath[0] === '/';
    var hasRootUrlPrefix = urlParts.length > 4;
    if (isRootPath && hasRootUrlPrefix) {
      var rootUrlPrefix =  urlParts[3];
      path = "/" + rootUrlPrefix + routeNameOrPath;
    }
  } else {
    // it's a route name!
